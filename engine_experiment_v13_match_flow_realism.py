...
The real-match benchmark showed that the candidate already tracks score and
shot distributions reasonably well, but produces too few corners and ordinary
fouls.  This layer fixes *causes* rather than targeting benchmark totals:

* defensive deflections from wide attacking actions can go behind for corners;
* blocks and goalkeeper parries can leave the field for a corner;
* ordinary contact around passes, carries and attacking actions can be called
  as a foul even when it is not a tactical-foul incident.

No score, goal quota, benchmark score, team identity or desired outcome is read
by this layer.  Existing referee/card logic remains authoritative once a foul is
called.
"""
...
class MatchEngineV13MatchFlowRealism(MatchEngineV13Penalties):
    ...
    def corner_turnover_probability(self, zone: Zone, reason: str, ctx: dict) -> float:
        """RNG-pure probability that a failed attacking action deflects behind."""
        ...
        # Crosses/cutbacks can be deflected behind even when the resulting ball
        # would otherwise be represented as a generic clearance.  Carries and
        # passes need a wide lane because a central tackle is much less likely
        # to leave via the goal line.
        ...
        box_bonus = 0.055 if zone.band == Band.BOX else 0.0
        wide_bonus = 0.055 if wide else 0.0
        probability = base + 0.11 * pressure + wide_bonus + box_bonus - 0.035 * space
    ...
    def _turnover(self, losing_team, actor, zone, reason, ctx, severity=0.5):
        ...
        if corner_p > 0.0 and self.rng.random() < corner_p:
            ...
            event.data["corner_cause"] = "deflection_or_clearance"
            event.data["failed_action"] = str(reason)
            event.data["corner_probability"] = round(corner_p, 4)
            return event
    ...
    def shot_corner_probability(self, p, event: Event) -> float:
        """RNG-pure chance that an already-resolved save/block exits for a corner."""
        ...
        if event.type == EventType.BLOCK:
            # The base engine already awards some blocked shots as corners.
            # This represents the remaining blocks that clip a defender and run
            # behind rather than becoming an immediate possession change.
    ...
    def _arm_corner_after_shot(self, team: int, p, event: Event, probability: float) -> Event:
        ...
        self.state.transition_boost = 0.0
        event.data["corner_awarded"] = True
        event.data["corner_probability"] = round(float(probability), 4)
        event.data["corner_cause"] = "save_parry" if event.type == EventType.SAVE else "blocked_deflection"
        return event
    ...
    def contact_foul_probability(
        ...
        """RNG-pure chance that ordinary contact is called before the action resolves."""
        base = float(self._CONTACT_ACTION_BASE.get(str(decision), 0.0))
        if base <= 0.0:
            return 0.0

        # Penalty-area contact is already modelled by dribble/handball/referee
        # systems.  Only ball-carrying actions receive a very small additional
        # chance here so this layer cannot flood the match with penalties.
        if zone.band == Band.BOX:
            if str(decision) not in {"carry", "dribble", "cutback"}:
                return 0.0
            base *= 0.18
    ...
    def _execute_decision(self, team, actor, zone, decision, ctx):
        ...
        if foul_p > 0.0 and self.rng.random() < foul_p:
            ...
            event.data["ordinary_contact"] = True
            event.data["contact_action"] = str(decision)
            event.data["contact_foul_probability"] = round(foul_p, 4)
            return event
